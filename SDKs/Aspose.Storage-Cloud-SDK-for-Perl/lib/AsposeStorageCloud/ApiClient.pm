...
sub call_api {
    ...
    $self->{ua}->timeout($self->{http_timeout} || $AsposeStorageCloud::Configuration::http_timeout); 
    $self->{ua}->agent($self->{http_user_agent} || $AsposeStorageCloud::Configuration::http_user_agent);
...
sub deserialize
{
    ...
    if (not defined $data) {
        ...
        my $_instance = use_module("AsposeStorageCloud::Object::$class")->new;
        if (ref $data eq "HASH") {
